...
// export async function onRequestPost(context) {
    ...
    try {
        ...
        if (slug) {
            ...
            // slug 已存在
            if (existUrl) {
                const updateInfo = await env.DB.prepare(`UPDATE links SET url = '${url}',ip = '${clientIP}',ua = '${userAgent}',create_time = '${formattedDate}' WHERE slug = '${slug}'`).run()

                return Response.json({ message: 'Slug update.' },{
                    headers: corsHeaders,
                    status: 200  
                })
            }
